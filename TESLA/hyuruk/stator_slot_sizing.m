...
bs1 = (pi*(Dis+2*hos*+2*hws)/Ns)-bts;   % [m] slot lower witdh bs1 (eq. 15.23)
...
Fmg = 1.2*g*10^-3*Bg_req/u0;           % [Aturns] airgap mmf (eq. 15.29)
                                       % note that carter coe. is taken 1.2
Hts = 3100;                            % [A/m] (table 15.4) interpolation
Fmts = Hts*(hs+hos+hws);               % [Aturns] stator tooth mmf (eq. 15.30)
Fmtr = Kst*Fmg - Fmts;                 % [Aturns] rotor tooth mmf (eq. 15.31)
hcs = (Dout-(Dis+2*(hos+hws+hs)))/2;   % [m] stator back iron height (eq. 15.32)
